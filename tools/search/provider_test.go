...
func TestProviderCountCol(t *testing.T) {
	...
	if p.countCol != "ctid" {
		t.Fatalf("Expected the default countCol to be %s, got %s", "ctid", p.countCol)
	}
...
func TestProviderExecNonEmptyQuery(t *testing.T) {
	...
	scenarios := []struct {
	...
	}{
		{
			...
			[]string{
				"SELECT COUNT(DISTINCT [[test.ctid]]) FROM `test` WHERE NOT (`test1` IS NULL)",
		...
		},
		{
			...
			[]string{
				"SELECT COUNT(DISTINCT [[test.ctid]]) FROM `test` WHERE ((NOT (`test1` IS NULL)) AND (((test2 != '' AND test2 IS NOT NULL)))) AND (test1 >= 2)",
		...
		},
		{
			...
			[]string{
				"SELECT COUNT(DISTINCT [[test.ctid]]) FROM `test` WHERE (NOT (`test1` IS NULL)) AND (((test3 != '' AND test3 IS NOT NULL)))",
